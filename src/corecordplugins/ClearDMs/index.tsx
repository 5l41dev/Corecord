...
async function clearAllDMs() {
    ...
    if (channels.length === 0) {
        showToast("No DMs to close", Toasts.Type.MESSAGE);
        return;
    }
    ...
    Alerts.show({
        ...
        body: (
            <div style={{ color: "#dbdee1" }}>
                Are you sure you want to close all your DMs? Your messages will not be deleted, but the conversations will disappear from your list.
            </div>
        ),
        onConfirm: async () => {
            showToast(`Closing ${channels.length} DMs...`, Toasts.Type.MESSAGE);
            let closedCount = 0;
